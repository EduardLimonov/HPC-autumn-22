...
scalar multiply(const struct MultiplyParams *params) {
    pthread_t *ids = calloc(params->num_of_threads, sizeof(pthread_t));
    scalar **results = calloc(params->num_of_threads, sizeof(scalar *));
    pthread_attr_t attr;
    pthread_attr_init(&attr);

    struct ThreadParams **list_of_thread_params = calloc(params->num_of_threads, sizeof(void *));

    for (int i = 0; i < params->num_of_threads; i++) {
        results[i] = calloc(1, sizeof(scalar));

        list_of_thread_params[i] = calloc(1, sizeof(struct ThreadParams));
        list_of_thread_params[i]->mul_params = params;
        list_of_thread_params[i]->my_id = i;
        list_of_thread_params[i]->result = results[i];

        int err = pthread_create(&ids[i], NULL, thread_multiply, list_of_thread_params[i]);
#ifdef VERBATIM
        if (err != 0)
    ...
    for (int i = 0; i < params->num_of_threads; i++) {
        ...
        sum += *(scalar *)results[i];
        free(results[i]);
        free(list_of_thread_params[i]);
    }

    free(ids);
    free(results);
    free(list_of_thread_params);
